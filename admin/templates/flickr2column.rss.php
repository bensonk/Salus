...
if ($feed->data) 
{
	$half = round($num / 2);
	echo '<div class="left">';
    foreach ($feed->get_items(0, $half) as $item) 
	{
        ...
        echo "<p><a href=\"htt".$image."\" class=\"thickbox\" title=\"".$item->get_title()."\"><img src=\"htt".$imagetn."\" alt=\"".$item->get_title()."\" /></a></p>\r\n";
    }
	echo '</div>';
	
	echo '<div class="right">';
	foreach ($feed->get_items($half, $half) as $item) 
	{
        $image = $item->get_description();
        $image = substr($image,strpos($image,'<img')+13);
        $image = trim(substr($image,0,strpos($image,"\" width")));  // pulls the url of the picture out of the description area and ignores everything else
